...
# Add tracked channel
async def add_tracked_channel(guild_id: int,
                              channel_id: int,
                              role_id: int,
                              *,
                              auto_role: bool = True,
                              drag_allowed: bool = True,
                              managed_role: bool = True) -> bool:
    ...
    async with AsyncSessionLocal() as session:
        ...
        stmt = stmt.on_conflict_do_update(index_elements=[
            VCTrackedChannel.guild_id, VCTrackedChannel.channel_id
        ],
                                          set_={
                                              "role_id": role_id,
                                              "enabled": True,
...
async def remove_tracked_channel(guild_id: int, channel_id: int) -> bool:
    async with AsyncSessionLocal() as session:
...
# Get tracked role
async def get_tracked_role(guild_id: int, channel_id: int) -> int | None:
    async with AsyncSessionLocal() as session:
...
# Get tracked channel
async def get_tracked_channel(guild_id: int, role_id: int) -> int | None:
...
async def is_channel_tracked(guild_id: int, channel_id: int) -> bool:
    async with AsyncSessionLocal() as session:
...
async def toggle_channel_tracking(guild_id: int, channel_id: int,
                                  enabled: bool) -> bool:
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            update(VCTrackedChannel).where(
                VCTrackedChannel.guild_id == guild_id,
                VCTrackedChannel.channel_id == channel_id).values(
                    enabled=enabled))
        await session.commit()
        return (getattr(result, "rowcount", 0) or 0) > 0
...
# Toggle auto role
async def toggle_auto_role(guild_id: int, channel_id: int,
                           enabled: bool) -> bool:
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            update(VCTrackedChannel).where(
...
# Toggle drag allowed
async def toggle_drag_allowed(guild_id: int, channel_id: int,
                              enabled: bool) -> bool:
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            update(VCTrackedChannel).where(
                VCTrackedChannel.guild_id == guild_id,
                VCTrackedChannel.channel_id == channel_id).values(
                    drag_allowed=enabled))
        await session.commit()
        return (getattr(result, "rowcount", 0) or 0) > 0


# Get role from channel
async def get_role_from_channel(guild_id: int, channel_id: int) -> int | None:
    return await get_tracked_role(guild_id, channel_id)


# Get channel from role
async def get_channel_from_role(guild_id: int, role_id: int) -> int | None:
    return await get_tracked_channel(guild_id, role_id)
