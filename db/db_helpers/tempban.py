...
async def set_tempban_role(
    ...
    async with AsyncSessionLocal() as session:
        stmt = build_insert_stmt(TempbanConfig, {
            "guild_id": guild_id,
            "role_id": role_id
        })
...
async def remove_tempban(*, guild_id: int, user_id: int,
    ...
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(
            update(TempbanRecord).where(TempbanRecord.guild_id == guild_id,
                                        TempbanRecord.user_id == user_id,
                                        TempbanRecord.active.is_(True)).values(
                                            active=False,
                                            moderator_id=moderator_id,
                                            updated_at=datetime.utcnow()))
...
async def get_active_tempbans(guild_id: int, ) -> List[TempbanRecord]:
    ...
    async with AsyncSessionLocal() as session:
        result = await session.scalars(
            select(TempbanRecord).where(TempbanRecord.guild_id == guild_id,
                                        TempbanRecord.active.is_(True)))
        return list(result)
    ...
    async with AsyncSessionLocal() as session:
        result = await session.scalars(
            select(TempbanRecord).where(
                TempbanRecord.active.is_(True),
                TempbanRecord.expires_at.is_not(None), TempbanRecord.expires_at
                <= datetime.utcnow()))
        return list(result)
