...
async def set_tempban_role(guild_id: int, role_id: int) -> None:
    async with AsyncSessionLocal() as session:
        stmt = build_insert_stmt(
            TempbanConfig, {"guild_id": guild_id, "role_id": role_id}
        )
        stmt = stmt.on_conflict_do_update(
            index_elements=[TempbanConfig.guild_id], set_={"role_id": role_id}
        )
...
async def get_tempban_role(guild_id: int) -> Optional[int]:
    async with AsyncSessionLocal() as session:
        return await session.scalar(
            select(TempbanConfig.role_id).where(TempbanConfig.guild_id == guild_id)
        )
...
async def add_tempban(
    *,
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Uses UTC timezone-aware replacement instead of deprecated utcnow()
        now_naive = datetime.now(timezone.utc).replace(tzinfo=None)

        stmt = build_insert_stmt(TempbanRecord, payload).on_conflict_do_update(
            index_elements=[TempbanRecord.guild_id, TempbanRecord.user_id],
            set_={
                "moderator_id": moderator_id,
                "tempban_reason": reason,
                "expires_at": expires_at,
...
# Remove tempban
async def remove_tempban(*, guild_id: int, user_id: int, moderator_id: int) -> bool:
    async with AsyncSessionLocal() as session:
        now_naive = datetime.now(timezone.utc).replace(tzinfo=None)
        result = await session.execute(
            ...
            .values(active=False, moderator_id=moderator_id, updated_at=now_naive)
        )
        await session.commit()
    ...
    async with AsyncSessionLocal() as session:
        result = await session.scalars(
            select(TempbanRecord).where(
                TempbanRecord.guild_id == guild_id, TempbanRecord.active.is_(True)
            )
        )
        return list(result)
    ...
    async with AsyncSessionLocal() as session:
        ...
        result = await session.scalars(
            select(TempbanRecord).where(
                ...
                TempbanRecord.expires_at <= now_naive,
            )
        )
        return list(result)
