...
async def create_permission_snapshots(
        ...
        snapshots: list[tuple[int, str, bool | None]]) -> None:
    if not snapshots:
        return
...
async def apply_channel_permissions(channel: discord.abc.GuildChannel,
                                    ...
                                    reason: str) -> bool:
    """Modifies channel overrides for designated validation roles. Returns True if all edits complete."""
    guild = channel.guild
    roles = await get_target_roles(guild)
    ...
    for role in roles:
        ...
        # Protect against KeyErrors on forum integrations during hide actions
        if isinstance(channel,
                      discord.ForumChannel) and "send_messages" in permissions:
            send_val = permissions.get("send_messages")
            overwrite.create_public_threads = send_val
            overwrite.create_private_threads = send_val

        # Execute API payload delivery
