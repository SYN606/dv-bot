            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSearchMember(); }} className="relative mb-6">
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 bg-slate-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-base text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                />
                <button
                  type="submit"
                  disabled={searchingMember || !search}
                  className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2.5 rounded-xl text-base font-semibold transition-all disabled:opacity-50"
                >
                  ...
                  {searchResults.map((res) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-mono">{res.id}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {memberPerms && (
              <div className="bg-slate-900/50 border border-white/10 rounded-2xl p-6 flex flex-col md:flex-row gap-8 items-start relative overflow-hidden">
                ...
                <div className="flex flex-col items-center gap-4 min-w-[200px] relative z-10">
                  <div className="relative">
                    ...
                    {memberPerms.user?.bot && (
                      <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-indigo-500 text-white text-xs font-bold px-2 py-0.5 rounded-full border-2 border-slate-900">
                        BOT
                      </div>
                    )}
                  </div>
                  <div className="text-center">
                    ...
                    <p className="text-sm text-slate-500 font-mono mt-1">{memberPerms.user?.id || search}</p>
                  </div>
                  ...
                  <div className="w-full flex flex-col gap-2 mt-2">
                    ...
                    <div className="w-full py-1.5 px-3 rounded-lg bg-slate-950 border border-white/5 flex justify-between items-center">
                      <span className="text-sm text-slate-400 font-semibold">Total Score</span>
                      <span className="text-base font-mono font-bold text-slate-200">{memberPerms.threatScore}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-5 w-full relative z-10">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2 pb-3 border-b border-white/5">
                        ...
                        {["red", "yellow", "green"].map(level => {
                          ...
                          return (
                            <div key={level} className="flex flex-col gap-2">
                              <h5 className={`text-base font-bold flex items-center gap-2 ${getRiskColor(level).split(' ')[0]}`}>
                                {emoji} {title}
                              </h5>
                              <div className="space-y-2">
                                {levelPerms.map((p, i) => (
                                  <div key={i} className="pl-6 border-l-2 border-white/10 ml-2">
                                    <div className="font-semibold text-slate-200 text-base flex items-center gap-2">
                                      {emoji} {p.permission}
                                    </div>
                  ...
                  <div className="pt-5 border-t border-white/5 mt-5">
                    <h4 className="text-base font-bold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                      <ShieldAlert className="w-4 h-4 text-rose-500" /> Punishment History
                    </h4>
                    
                    {!memberPerms.history || memberPerms.history.length === 0 ? (
                      <div className="bg-slate-900/50 border border-emerald-500/20 text-emerald-400 p-4 rounded-xl text-base font-medium flex items-center gap-2">
                        <ShieldCheck className="w-5 h-5" /> This user has a clean record.
                      </div>
                    ) : (
                      <div className="space-y-3">
                        <div className="text-sm text-slate-400 mb-2 font-semibold">Total Infractions: {memberPerms.history.length}</div>
                        {memberPerms.history.map((record, idx) => (
                          <div key={idx} className="bg-slate-900 border border-white/5 p-4 rounded-xl flex flex-col gap-2">
                            ...
                            <p className="text-base text-slate-300 mt-1">{record.reason}</p>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2"
                title="Export to CSV"
              >
                Export Report
              </button>
              <button
                ...
                className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-semibold transition-colors flex items-center gap-2 shadow-md"
              >
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <select 
                      ...
                      className="bg-slate-900 border border-white/10 text-sm text-slate-300 rounded-lg px-2 py-1 focus:outline-none"
                    >
                    ...
                    <div key={member.id} className={`bg-slate-950/50 border rounded-xl p-4 flex flex-col gap-3 transition-colors hover:bg-slate-900/50 ${
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 flex flex-col">
                            <span className="font-semibold text-slate-200 flex items-center gap-2 text-base">
                              {member.username}
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 rounded-lg text-sm font-bold transition-colors"
                        >
                          Audit
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-1 pt-3 border-t border-white/5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-slate-900 ${
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-400 font-mono font-bold bg-slate-900 px-1.5 py-0.5 rounded-md ml-1 border border-white/5">
                ...
                {memberTotalPages > 1 && (
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                    <button
                      ...
                      className="px-3 py-1 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 rounded text-sm font-semibold text-slate-300"
                    >
                      Prev
                    </button>
                    ...
                    <div key={role.id} className="bg-slate-950/50 border border-white/5 rounded-xl p-4 flex flex-col gap-3 transition-colors hover:bg-slate-900/50">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-semibold text-slate-200 text-base">{role.name}</span>
                            <span className="text-xs text-slate-500 font-mono">{role.id}</span>
                          </div>
                        </div>
                        <span className="text-sm font-bold text-slate-400 bg-slate-900 px-2 py-1 rounded-md border border-white/5 flex items-center gap-1.5">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-2 border-t border-white/5">
                        {role.permissions.map((p, i) => (
                          <span key={i} className={`px-2 py-0.5 rounded text-xs font-bold border ${getRiskColor(p.level)} flex items-center gap-1`}>
