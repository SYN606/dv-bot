...
class Purge(BaseAdminCog):
    """Cog for performing bulk message deletion in guild channels."""
    ...
    async def _reply(
        ...
        try:
            ...
            system_emoji = EMOJIS.get(emoji_key) or EMOJIS.get("warning") or ""

            formatted_description = (
                f"{system_emoji} {description}".strip()
                if system_emoji
                else description
            )
    ...
    async def collect_messages(
        ...
        *,
        ...
        scan_limit = self.MAX_SCAN if member else min(amount + 50, self.MAX_SCAN)
    ...
    async def delete_messages(
        self, *, channel: GuildChannel, messages: list[discord.Message]
    ) -> int:
    ...
    async def purge(self, ctx: commands.Context, *args: str) -> None:
        ...
        target_string = (
            f"from {member.mention}" if member else "from this channel"
        )
        success_icon = EMOJIS.get("success") or "✅"
        await self._reply(
            ctx,
            "Messages Purged",
            f"{success_icon} Successfully deleted **{deleted_count}** messages {target_string}.",
            level="SUCCESS",
            delete_after=5,
        )
        ...
        try:
            await send_mod_log(
                ...
                description=f"Bulk deleted **{deleted_count}** messages in {channel.mention}.",
                level="SUCCESS",
                actor=moderator,
                target=member,
                extra_fields={
