...
class HelpDropdown(discord.ui.Select):
    ...
    def __init__(self, categories: list[dict], author_id: int,
        ...
        options = [
            discord.SelectOption(label=cat["name"],
                                 value=cat["id"],
                                 emoji=cat.get("emoji")
                                 or CATEGORY_EMOJIS.get(cat["id"], "🔹"),
                                 description=cat.get(
                                     "description",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        desc += "Browse the available commands below. Use `help <command>` for specific syntax.\n\n"

        arrow = EMOJIS.get("arrow_point", "🔹")
        for c in category_data["commands"]:
            prefix = "/" if c.get("is_slash") else self.ctx_prefix
            name = c['name'].replace("/", "")  # Ensure no double slashes
            aliases = f" *[Aliases: {', '.join(c['aliases'])}]*" if c.get(
                "aliases") else ""
...
class Help(commands.Cog):
    ...
    async def _get_authorized_help_tree(self, guild, channel, is_admin):
        restricted = set(await get_restricted_commands(
            guild.id, channel.id)) if guild and channel else set()
        filtered_categories = []
    ...
    async def help(self, ctx: commands.Context, command_name: str = None): # type: ignore
        ...
        desc = (
            f"### {EMOJIS.get("ping")} Welcome...\n"
            "Welcome to the command directory. Select a module from the dropdown below to view category-specific actions.\n\n"
            "• **Need specific info?** Use `!help <command_name>`\n"
        ...
        embed = make_embed(title="Digital Vigital • Main Menu",
                           ...
                           level="INFO")
        if BANNER_GIF: embed.set_image(url=BANNER_GIF)
        view = HelpDropdownView(tree, ctx.author.id, current_prefix)
