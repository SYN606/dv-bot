...
# BUILD TITLE
def _build_title(title: str, emoji: Optional[str]) -> str:
...
def make_embed(*,
    ...
    title_text = _build_title(title, emoji)

    embed = discord.Embed(title=_safe(title_text, TITLE_LIMIT),
                          description=_safe(description, DESCRIPTION_LIMIT),
                          color=color,
                          url=_safe_url(url))
    ...
    if author:
        ...
        embed.set_author(name=_safe(author, AUTHOR_LIMIT),
    ...
    # footer
    if footer:
        embed.set_footer(text=_safe(footer, FOOTER_LIMIT),
