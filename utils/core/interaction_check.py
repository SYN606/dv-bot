...
async def command_toggle_check(interaction: discord.Interaction, ) -> bool:
    ...
    embed = make_embed(title="Command Restricted",
...
def invalidate_command_restrict_cache(guild_id: int, channel_id: int,
                                      command_name: str) -> None:

    _RESTRICT_CACHE.pop((guild_id, channel_id, command_name.lower()), None)


def clear_guild_restrict_cache(guild_id: int, ) -> None:
    keys = [key for key in _RESTRICT_CACHE if key[0] == guild_id]
    for key in keys:
        _RESTRICT_CACHE.pop(key, None)
