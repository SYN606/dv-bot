...
async def delete_old_sticky(
    ...
    last_delete = _DELETE_FAILSAFE.get(
        channel.id,
        0,
    )
...
async def process_sticky(
    ...
    *,
    ...
    lock = _CHANNEL_LOCKS.setdefault(
        channel.id,
        asyncio.Lock(),
    )
    ...
    async with lock:
        ...
        if payload.message_id:
            ...
            await delete_old_sticky(
                channel,
                payload.message_id,
            )

        # send new sticky
        message = await send_sticky(
            channel,
            payload,
        )
