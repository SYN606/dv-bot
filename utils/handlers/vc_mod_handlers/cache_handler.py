...
VC_ROLE_CACHE: dict[int, dict[int, dict]] = {}


async def build_guild_cache(guild_id: int, ) -> None:
    ...
    VC_ROLE_CACHE[guild_id] = {
        ...
        for item in data if item.enabled
    }


def clear_guild_cache(guild_id: int, ) -> None:
    VC_ROLE_CACHE.pop(guild_id, None)


def set_cache_mapping(guild_id: int,
                      channel_id: int,
                      role_id: int,
                      *,
                      enabled: bool = True,
...
def remove_cache_mapping(guild_id: int, channel_id: int) -> None:
    guild_cache = VC_ROLE_CACHE.get(guild_id)
    if not guild_cache:
        return
    guild_cache.pop(channel_id, None)


# Get cached data
def get_cached_data(guild_id: int, channel_id: int) -> dict | None:
    guild_cache = VC_ROLE_CACHE.get(guild_id)
    if not guild_cache:
        return None
...
def get_cached_role(guild_id: int, channel_id: int) -> int | None:
    data = get_cached_data(guild_id, channel_id)
    if not data:
        return None
