...
from utils.handlers.media_only import build_media_only_sticky_embed, STICKY_TAG
from db.db_helpers.media_only import enable_media_only, disable_media_only, is_media_only


async def remove_media_only_sticky(channel: discord.TextChannel) -> None:
    try:
        async for message in channel.history(limit=50):
            if message.embeds and message.embeds[0].footer and STICKY_TAG in (
                    message.embeds[0].footer.text or ""):
                try:
...
class MediaOnlyView(discord.ui.View):
    ...
    @discord.ui.button(label="Enable",
    ...
    async def disable(self, interaction: discord.Interaction, _):
        ...
        if not await disable_media_only(self.guild_id, self.channel.id):
            return await interaction.edit_original_response(embed=make_embed(
        ...
        await interaction.edit_original_response(embed=make_embed(
            ...
            description=
            f"{EMOJIS['success']} Restrictions removed.\n\n{EMOJIS['arrow_point']} Normal messages allowed.",
            level="SUCCESS"),
                                                 view=self)
