    ...
    async def load_startup_modules(self) -> None:
        ...
        for root, _, files in os.walk(base_path):
            for file in files:
                ...
                try:
                    startup_module = __import__(module, fromlist=["startup"])
                    startup_func = getattr(startup_module, "startup", None)

                    if startup_func is None:
                        logger.warning(
                            f"[STARTUP] {module} has no 'startup' function")
                        continue

                    if not inspect.iscoroutinefunction(startup_func):
                        logger.warning(
                            f"[STARTUP] {module}.startup is not async")
                        continue
...
def main() -> None:
    while True:
        ...
        try:
            ...
            if exc.status == 429:
                logger.warning(
                    "[RATE LIMIT] Hit Discord rate limit. Backing off for 60 seconds..."
                )
                time.sleep(60)
                continue
            ...
            logger.info("[RESTART] Restarting loop iteration in 30 seconds...")
            time.sleep(30)
